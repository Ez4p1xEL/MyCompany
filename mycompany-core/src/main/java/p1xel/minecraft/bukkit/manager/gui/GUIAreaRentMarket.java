package p1xel.minecraft.bukkit.manager.gui;
...
import p1xel.minecraft.bukkit.manager.area.RentableCompanyArea;
import p1xel.minecraft.bukkit.util.permission.Permission;
import p1xel.minecraft.bukkit.util.storage.Locale;
...
public class GUIAreaRentMarket extends GUIAbstract implements InventoryHolder {
    ...
    public GUIAreaRentMarket(UUID playerUniqueId, String action, int page) {
    ...
    public void init() {
        ...
        Inventory inventory = Bukkit.createInventory(this, 45, Locale.getMessage("menu.area-rent-market.title").replaceAll("%company%", companyName));
        ...
        List<RentableCompanyArea> areas = new ArrayList<>(areaManager.getMarketManager().getRentMarketAreas());
        ...
        area_info_meta.setDisplayName(Locale.getMessage("menu.area-rent-market.items.area_info.display_name").replaceAll("%company%", companyName));
        List<String> area_info_lore = Locale.yaml.getStringList("menu.area-rent-market.items.area_info.lore").stream()
        ...
        if (hasNextPage) {
            ...
            ItemStack item = new ItemStack(Material.matchMaterial(Locale.getMessage("menu.area-rent-market.items.next_page.material")));
            ItemMeta meta = item.getItemMeta();
            meta.setDisplayName(Locale.getMessage("menu.area-rent-market.items.next_page.display_name"));
            List<String> lore = Locale.yaml.getStringList("menu.area-rent-market.items.next_page.lore").stream()
        ...
        if (hasPreviousPage) {
            ...
            ItemStack item = new ItemStack(Material.matchMaterial(Locale.getMessage("menu.area-rent-market.items.previous_page.material")));
            ItemMeta meta = item.getItemMeta();
            meta.setDisplayName(Locale.getMessage("menu.area-rent-market.items.previous_page.display_name"));
            List<String> lore = Locale.yaml.getStringList("menu.area-rent-market.items.previous_page.lore").stream()
                    .map(line -> ChatColor.translateAlternateColorCodes('&', line)).collect(Collectors.toList());
            meta.setLore(lore);
            PersistentDataContainer container = meta.getPersistentDataContainer();
        ...
        ItemStack item = new ItemStack(Material.matchMaterial(Locale.getMessage("menu.area-rent-market.items.back_to_main.material")));
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(Locale.getMessage("menu.area-rent-market.items.back_to_main.display_name"));
        List<String> lore = Locale.yaml.getStringList("menu.area-rent-market.items.back_to_main.lore").stream()
                .map(line -> ChatColor.translateAlternateColorCodes('&', line)).collect(Collectors.toList());
        meta.setLore(lore);
        PersistentDataContainer container = meta.getPersistentDataContainer();
        ...
        if (name.startsWith("company:")) {
            ...
            if (action.equalsIgnoreCase("rent")) {
                if (type == ClickType.RIGHT) {
                    ...
                    if (!userManager.hasPermission(playerUniqueId, Permission.AREA_RENT)) {
                        return false;
                    }
                    ...
                    new PersonalAPI(playerUniqueId).rentArea(areaCompanyUniqueId, areaName);
                }
                return false;
            }
        }
    ...
    public void update(List<RentableCompanyArea> areas) {
        int slot = 20;
        for (RentableCompanyArea area : areas) {
            if (slot == 25) {
                slot = 29;
            }
            ...
            String material = Locale.getMessage("menu.area-rent-market.items.area.material");
            ...
            meta.setDisplayName(Locale.getMessage("menu.area-rent-market.items.area.display_name").replaceAll("%company%", companyName).replaceAll("%area%", areaName));
            List<String> lore_list = Locale.yaml.getStringList("menu.area-rent-market.items.area.lore").stream()
                    .map(line -> ChatColor.translateAlternateColorCodes('&', line))
                    .map(line -> line.replaceAll("%company%", companyName)
                    .replaceAll("%area%", areaName)
                    .replaceAll("%price%", String.valueOf(area.getRentPrice())))
                    .collect(Collectors.toList());
